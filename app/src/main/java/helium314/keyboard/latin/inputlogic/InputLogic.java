...
public final class InputLogic {
    ...
    // TODO: This boolean is persistent state and causes large side effects at
    // unexpected times.
     ...
     *                                    view.
     * @param dictionaryFacilitator       facilitator for getting suggestions and
     ...
     * <p>
     * Call this when input starts or restarts in some editor (typically, in
     * onStartInputView).
     *
     * @param combiningSpec  the combining spec string for this subtype (from extra
     *                       value)
    ...
    public void startInput(final String combiningSpec, final SettingsValues settingsValues) {
        ...
        if (!mWordComposer.getTypedWord().isEmpty()) {
            // For messaging apps that offer send button, the IME does not get the
            // opportunity
        ...
        // before bringing up keyboard)
        // editorInfo.initialSelStart is not the actual cursor position, so we try using
        // some heuristics to find the correct position.
    ...
    public void onOrientationChange(final SettingsValues settingsValues) {
        // If !isComposingWord, #commitTyped() is a no-op, but still, it's better to
        // avoid
        ...
        if (mWordComposer.isComposingWord()) {
            ...
            // suggestionsSpan will be added. This will allow resuming on the same
            // suggestions
     ...
     * <p>
     * This is triggered by keys that input many characters at once, like the ".com"
     * key or
    ...
    public InputTransaction onTextInput(final SettingsValues settingsValues, final Event event,
        ...
        if (mWordComposer.isComposingWord()) {
            ...
            addToHistoryIfEmoji(rawText, settingsValues); // add emoji before resetting, otherwise lastComposedWord is
                                                          // empty
            resetComposingState(true /* alsoResetLastComposedWord */);
        }
    ...
    public InputTransaction onPickSuggestionManually(final SettingsValues settingsValues,
        ...
        // the current batch input text and there is no need for a phantom space.
                ...
                // when a commit was reverted and user chose a different suggestion, we don't
                // want
        ...
        // TODO: We should not need the following branch. We should be able to take the
        // same
        // code path as for other kinds, use commitChosenWord, and do everything
        // normally. We will
        // however need to reset the suggestion strip right away, because we know we
        // can't take
        ...
        // If we're not showing the "Touch again to save", then update the suggestion
        // strip.
        // That's going to be predictions (or punctuation suggestions), so
        // INPUT_STYLE_NONE.
    ...
    /**
     * Consider an update to the cursor position. Evaluate whether this update has
     * happened as
    ...
    public boolean onUpdateSelection(final int oldSelStart, final int oldSelEnd, final int newSelStart,
        ...
        // that
        // does not get reflected in the oldSel{Start,End} parameters to the next call
        // to
        // onUpdateSelection. In this case, we may have set a composition, and when
        // we're here
        // we realize we shouldn't have. In theory, in this case,
        // selectionChangedOrSafeToReset
        // should be true, but that is if the framework had taken that wrong cursor
        // position
        // into account, which means we have to reset the entire composing state
        // whenever there
        ...
        if (hasOrHadSelection || !settingsValues.needsToLookupSuggestions()
            ...
            // cursor back to where it was. Latin IME could then fix the position of the
            // cursor
            // again, but the asynchronous nature of the calls results in this wreaking
            // havoc
            ...
            // suggestions strip in a few milliseconds, so if we cleared the suggestion
            // strip here
            // we'd have the suggestion strip noticeably janky. To avoid that, we don't
            // clear
            // it here, which means we'll keep outdated suggestions for a split second but
            // the
            // visual result is better.
            resetEntireInputState(newSelStart, newSelEnd, false /* clearSuggestionStrip */);
            // If the user is in the middle of correcting a word, we should learn it before
            // moving
            // the cursor away.
    ...
    /**
     * React to a code input. It may be a code point to insert, or a symbolic value
     * that influences
     * the keyboard behavior.
     * <p>
     * Typically, this is called whenever a key is pressed on the software keyboard.
     * This is not
     * the entry point for gesture input; see the onBatchInput* family of functions
     * for this.
     *
     * @param settingsValues    the current settings values.
     * @param event             the event to handle.
     * @param keyboardShiftMode the current shift mode of the keyboard, as returned
     *                          by
     *                          {@link helium314.keyboard.keyboard.KeyboardSwitcher#getKeyboardShiftMode()}
     * @return the complete transaction object
     */
    public InputTransaction onCodeInput(final SettingsValues settingsValues,
        ...
        mConnection.beginBatchEdit();
        if (!mWordComposer.isComposingWord()) {
            // TODO: is this useful? It doesn't look like it should be done here, but rather
            // after
            // a word is committed.
        ...
        // Try to record the word being corrected when the user enters a word character
        // or
        // the backspace key.
    ...
    public void onStartBatchInput(final SettingsValues settingsValues,
        ...
        if (mWordComposer.isComposingWord()) {
            ...
            if (selectedText != null)
                // set selected text as rejected to avoid glide typing resulting in exactly the
                // selected word again
        ...
        if (Character.isLetterOrDigit(codePointBeforeCursor)
            ...
            if (!autoShiftHasBeenOverriden) {
                ...
                // keyboard unless it has been overridden manually. This is happening for
                // example
                // after typing some letters and a period, then gesturing; the keyboard is not
                // in
    ...
    /*
     * The sequence number member is only used in onUpdateBatchInput. It is
     * increased each time
     * auto-commit happens. The reason we need this is, when auto-commit happens we
     * trim the
     * input pointers that are held in a singleton, and to know how much to trim we
     * rely on the
     * results of the suggestion process that is held in mSuggestedWords.
     * However, the suggestion process is asynchronous, and sometimes we may enter
     * the
     * onUpdateBatchInput method twice without having recomputed suggestions yet, or
     * having
     * received new suggestions generated from not-yet-trimmed input pointers. In
     * this case, the
     * mIndexOfTouchPointOfSecondWords member will be out of date, and we must not
     * use it lest we
     * remove an unrelated number of pointers (possibly even more than are left in
     * the input
     * pointers, leading to a crash).
     * To avoid that, we increase the sequence number each time we auto-commit and
     * trim the
     * input pointers, and we do not use any suggested words that have been
     * generated with an
    ...
    public void setSuggestedWords(final SuggestedWords suggestedWords) {
        ...
        if (mIsAutoCorrectionIndicatorOn != newAutoCorrectionIndicator && mWordComposer.isComposingWord()) {
            ...
            // TODO: when called from an updateSuggestionStrip() call that results from a
            // posted
            // message, this is called outside any batch edit. Potentially, this may result
            // in some
            // janky flickering of the screen, although the display speed makes it unlikely
            // in
            // the practice.
     ...
     * <p>
     * Consumed events represent events that have already been consumed, typically
     * by the
     * combining chain.
     *
     * @param event            The event to handle.
    ...
    private void handleConsumedEvent(final Event event, final InputTransaction inputTransaction) {
        // A consumed event may have text to commit and an update to the composing
        // state, so
        // we evaluate both. With some combiners, it's possible than an event contains
        // both
        ...
        if (mWordComposer.isComposingWord()) {
            // Check if we need to insert automatic space before starting to compose (e.g.,
            // after suggestion pickup)
     ...
     * <p>
     * A functional event is a special key, like delete, shift, emoji, or the
     * settings key.
     * Non-special keys are those that generate a single code point.
     * This includes all letters, digits, punctuation, separators, emoji. It
     * excludes keys that
     * manage keyboard-related stuff like shift, language switch, settings, layout
     * switch, or
    ...
    private void handleFunctionalEvent(final Event event, final InputTransaction inputTransaction,
        ...
        switch (keyCode) {
            ...
            case KeyCode.SHIFT_ENTER:
                // todo: try using sendDownUpKeyEventWithMetaState() and remove the key code
                // maybe
            ...
            case KeyCode.MULTIPLE_CODE_POINTS:
                ...
                // this introduces issues like space being added behind cursor, or input
                // deleting
            ...
            case KeyCode.CLIPBOARD_SELECT_WORD:
                mConnection.selectWord(inputTransaction.getSettingsValues().mSpacingAndPunctuations,
            ...
            case KeyCode.WORD_LEFT:
                sendDownUpKeyEventWithMetaState(
                        ScriptUtils.isScriptRtl(currentKeyboardScript) ? KeyEvent.KEYCODE_DPAD_RIGHT
                                : KeyEvent.KEYCODE_DPAD_LEFT,
                        KeyEvent.META_CTRL_ON | event.getMetaState());
                break;
            case KeyCode.WORD_RIGHT:
                sendDownUpKeyEventWithMetaState(
                        ScriptUtils.isScriptRtl(currentKeyboardScript) ? KeyEvent.KEYCODE_DPAD_LEFT
            ...
            case KeyCode.MOVE_START_OF_PAGE:
                ...
                sendDownUpKeyEventWithMetaState(KeyEvent.KEYCODE_MOVE_HOME,
                        KeyEvent.META_CTRL_ON | event.getMetaState());
                if (mConnection.getExpectedSelectionStart() == selectionStart1
                        && mConnection.getExpectedSelectionEnd() == selectionEnd1) {
            ...
            case KeyCode.MOVE_END_OF_PAGE:
                ...
                sendDownUpKeyEventWithMetaState(KeyEvent.KEYCODE_MOVE_END,
                        KeyEvent.META_CTRL_ON | event.getMetaState());
                if (mConnection.getExpectedSelectionStart() == selectionStart2
                        && mConnection.getExpectedSelectionEnd() == selectionEnd2) {
                    // unchanged, try fallback e.g. for compose fields that don't care about ctrl +
                    // end
                    // we just move to a very large index, and hope the field is prepared to deal
                    // with this
                    // getting the actual length of the text for setting the correct position can be
                    // tricky for some apps...
                    try {
                        final int newStart = (event.getMetaState() & KeyEvent.META_SHIFT_MASK) != 0 ? selectionStart2
                                : Integer.MAX_VALUE;
            ...
            case KeyCode.VOICE_INPUT:
                ...
                // {@link KeyboardSwitcher#onEvent(Event)}, or {@link
            ...
            default:
                if (KeyCode.INSTANCE.isModifier(keyCode))
                    return; // continuation of previous switch case above, but modifiers are held in a
                            // separate place
                ...
                if (event.getMetaState() != 0 && event.getCodePoint() >= 32) {
                    // Try handling it as normal key event, this essentially just ignore the meta
                    // state.
                    // The conversion is good, as this way we are able to use e.g. ctrl + V. But if
                    // we don't have a codePointToKeyEventCode
                    // for that key, we will just input the normal key now, e.g. ctrl + @ will
                    // (probably) do nothing, but ctrl + _ will input _
                    // Maybe we should just return instead?
                    handleNonFunctionalEvent(event, inputTransaction, handler);
                    return;
                }
                // unknown event
                Log.e(TAG, "unknown event, key code: " + keyCode + ", codepoint " + event.getCodePoint() + ", meta: "
                        + event.getMetaState());
     ...
     * <p>
     * These events are generally events that cause input, but in some cases they
     * may do other
     ...
     * @param inputTransaction The transaction in progress.
     */
    private void handleNonFunctionalEvent(final Event event, final InputTransaction inputTransaction,
            final LatinIME.UIHandler handler) {
        inputTransaction.setDidAffectContents();
    ...
    private void handleNonSpecialCharacterEvent(final Event event,
        ...
        // otherwise it would work too, but whenever a separator is entered, the word is
        // not selected
        // until the next character is entered, and the word is added to history
        // -> the changing selection would be confusing, and adding partial URLs to
        // history is probably bad
        if (Character.getType(codePoint) == Character.OTHER_SYMBOL
                || (Character.getType(codePoint) == Character.UNASSIGNED && StringUtils.mightBeEmoji(codePoint)) // outdated
                                                                                                                 // java
                                                                                                                 // doesn't
                                                                                                                 // detect
                                                                                                                 // some
                ...
                || (sv.isWordSeparator(codePoint)
                        && (Character.isWhitespace(codePoint) // whitespace is always a separator
                                || !textBeforeCursorMayBeUrlOrSimilar(sv, false) // if text before is not URL or
                                                                                 // similar, it's a separator
                                || (codePoint == '/' && mWordComposer.lastChar() == '/') // break composing at 2
                                                                                         // consecutive slashes
                        ))) {
    ...
    private void addToHistoryIfEmoji(final String text, final SettingsValues settingsValues) {
        if (mLastComposedWord == LastComposedWord.NOT_A_COMPOSED_WORD // we want a last composed word, also to avoid
                                                                      // storing consecutive emojis
                || mWordComposer.isComposingWord() // emoji will be part of the word in this case, better do nothing
                || !settingsValues.mBigramPredictionEnabled // this is only for next word suggestions, so they need to
                                                            // be enabled
                || settingsValues.mIncognitoModeEnabled
                || !settingsValues.isSuggestionsEnabledPerUserSettings() // see comment in
        ...
        // commit emoji to dictionary, so it ends up in history and can be suggested as
        // next word
    ...
    private void handleNonSeparatorEvent(final Event event, final SettingsValues settingsValues,
        ...
        // handleNonSpecialCharacterEvent which has the same name as other handle*
        // methods but is
        // not the same.
        ...
        if (settingsValues.mUrlDetectionEnabled && settingsValues.needsToLookupSuggestions()
                ...
                && settingsValues.mSpacingAndPunctuations
                        .isSometimesWordConnector(mConnection.getCodePointBeforeCursor())
        ...
        if (mWordComposer.isCursorInFrontOfComposingWord()) {
            // we add something in front of the composing word, this is likely for adding
            // something
        ...
        // We want to find out whether to start composing a new word with this
        // character. If so,
        // we need to reset the composing state and switch isComposingWord. The order of
        // the
        // tests is important for good performance.
        // We only start composing if we're not already composing.
        if (!isComposingWord
                // We only start composing if this is a word code point. Essentially that means
                // it's a
                // a letter or a word connector.
                && settingsValues.isWordCodePoint(codePoint)
                // We never go into composing state if suggestions are not requested.
                && settingsValues.needsToLookupSuggestions() &&
                // In languages with spaces, we only start composing a word when we are not
                // already
                // in the middle or at the end of a word. In languages without spaces, the above
                // conditions are sufficient.
                // NOTE: If the InputConnection is slow, we skip the text-after-cursor check
                // since it
                // can incur a very expensive getTextAfterCursor() lookup, potentially making
                // the
            ...
            // Reset entirely the composing state anyway, then start composing a new word
            // unless
            // the character is a word connector. The idea here is, word connectors are not
            // separators and they should be treated as normal characters, except in the
            // first
        ...
        if (isComposingWord) {
            ...
            } else if ((settingsValues.mInputAttributes.mInputType
                    & InputType.TYPE_MASK_CLASS) != InputType.TYPE_CLASS_TEXT
                    && codePoint >= '0' && codePoint <= '9') {
                // weird issue when committing text:
                // https://github.com/Helium314/HeliBoard/issues/585
                // but at the same time we don't always want to do it for numbers because it
                // might interfere with url detection
    ...
    private void handleSeparatorEvent(final Event event, final InputTransaction inputTransaction,
        ...
        if (SpaceState.PHANTOM != inputTransaction.getSpaceState()) {
            ...
            needsPrecedingSpace = settingsValues.isUsuallyPrecededBySpace(codePoint)
                    || StringUtilsKt.isEmoji(codePoint);
        }
        ...
        if (tryPerformDoubleSpacePeriod(event, inputTransaction)) {
            ...
            if (SpaceState.PHANTOM == inputTransaction.getSpaceState()
                ...
                // If we are in phantom space state, and the user presses a separator, we want
                // to
                // stay in phantom space state so that the next keypress has a chance to add the
                // space. For example, if I type "Good dat", pick "day" from the suggestion
                // strip
                // then insert a comma and go on to typing the next word, I want the space to be
                // inserted automatically before the next word, the same way it is when I don't
                // input the comma. Also when closing a quote the phantom state should be
                // preserved.
                // The case is a little different if the separator is a space stripper. Such a
                // separator does not normally need a space on the right (that's the difference
                // between swappers and strippers), so we should not stay in phantom space state
                // if
            ...
            if (isInlineEmojiSearchAction()) {
                ...
                // Set punctuation right away. onUpdateSelection will fire but tests whether it
                // is
    ...
    private void handleBackspaceEvent(final Event event, final InputTransaction inputTransaction) {
        ...
        // user types
        // backspace then some other character very fast. However, in the case of
        // backspace key
        // repeat, this can lead to flashiness when the cursor flies over positions
        // where the
        // shift state should be updated, so if this is a key repeat, we update after a
        // small delay.
        // Then again, even in the case of a key repeat, if the cursor is at start of
        // text, it
        // can't go any further back, so we can update right away even if it's a key
        // repeat.
        ...
        if (mWordComposer.isCursorFrontOrMiddleOfComposingWord()) {
            ...
            resetEntireInputState(mConnection.getExpectedSelectionStart(),
                    mConnection.getExpectedSelectionEnd(), true /* clearSuggestionStrip */);
            // When we exit this if-clause, mWordComposer.isComposingWord() will return
            // false.
        }
        if (mWordComposer.isComposingWord()) {
            ...
            if (mLastComposedWord.canRevertCommit()
                ...
                // Restart suggestions when backspacing into a reverted word. This is required
                // for
                // the final corrected word to be learned, as learning only occurs when
                // suggestions
                // are active.
                //
            ...
            if (false && mEnteredText != null && mConnection.sameAsTextBeforeCursor(mEnteredText)) {
                ...
                mEnteredText = null;
                // If we have mEnteredText, then we know that mHasUncommittedTypedChars ==
                // false.
    ...
    void unlearnWord(final String word, final SettingsValues settingsValues, final int eventType) {
        final NgramContext ngramContext = mConnection
                .getNgramContextFromNthPreviousWord(settingsValues.mSpacingAndPunctuations, 2);
    ...
    public boolean isDoubleSpacePeriodCountdownActive(final InputTransaction inputTransaction) {
        return inputTransaction.getTimestamp()
                - mDoubleSpacePeriodCountdownStart < inputTransaction.getSettingsValues().mDoubleSpacePeriodTimeout;
    }

    /**
     * Apply the double-space-to-period transformation if applicable.
     * <p>
     * The double-space-to-period transformation means that we replace two spaces
     * with a
     * period-space sequence of characters. This typically happens when the user
     * presses space
     * twice in a row quickly.
     * This method will check that the double-space-to-period is active in settings,
     * that the
     * two spaces have been input close enough together, that the typed character is
     * a space
     * and that the previous character allows for the transformation to take place.
     * If all of
     * these conditions are fulfilled, this method applies the transformation and
     * returns true.
     * Otherwise, it does nothing and returns false.
     *
     * @param event            The event to handle.
     * @param inputTransaction The transaction in progress.
     * @return true if we applied the double-space-to-period transformation, false
     *         otherwise.
     */
    private boolean tryPerformDoubleSpacePeriod(final Event event,
            final InputTransaction inputTransaction) {
        // Check the setting, the typed character and the countdown. If any of the
        // conditions is
        ...
        // We only do this when we see one space and an accepted code point before the
        // cursor.
        // The code point may be a surrogate pair but the space may not, so we need 3
        // chars.
        ...
        // We know there is a space in pos -1, and we have at least two chars. If we
        // have only two
        // chars, isSurrogatePairs can't return true as charAt(1) is a space, so this is
        // fine.
        ...
        if (canBeFollowedByDoubleSpacePeriod(firstCodePoint)) {
            cancelDoubleSpacePeriodCountdown();
            mConnection.deleteTextBeforeCursor(1);
            final String textToInsert = inputTransaction
                    .getSettingsValues().mSpacingAndPunctuations.mSentenceSeparatorAndSpace;
    ...
    /**
     * Returns whether this code point can be followed by the double-space-to-period
     * transformation.
     * <p>
     * See #maybeDoubleSpaceToPeriod for details.
     * Generally, most word characters can be followed by the double-space-to-period
     * transformation,
     * while most punctuation can't. Some punctuation however does allow for this to
     * take place
     * after them, like the closing parenthesis for example.
     *
     * @param codePoint the code point after which we may want to apply the
     *                  transformation
    ...
    private void performRecapitalization(final SettingsValues settingsValues) {
        ...
        if (numCharsSelected > Constants.MAX_CHARACTERS_FOR_RECAPITALIZATION) {
            // We bail out if we have too many characters for performance reasons. We don't
            // want
        ...
        if (!mRecapitalizeStatus.isStarted()
            ...
            final CharSequence selectedText = mConnection.getSelectedText(0 /* flags, 0 for no styles */);
            if (TextUtils.isEmpty(selectedText))
                return; // Race condition with the input connection
    ...
    private void performAdditionToUserHistoryDictionary(final SettingsValues settingsValues,
        ...
        // If correction is not enabled, we don't add words to the user history
        // dictionary.
        // That's to avoid unintended additions in some sensitive fields, or fields that
        // expect to receive non-words.
        // mInputTypeNoAutoCorrect changed to !isSuggestionsEnabledPerUserSettings
        // because this was cancelling learning way too often
        ...
        if (settingsValues.mIncognitoModeEnabled) {
            // still adjust confidences, otherwise incognito input fields can be very
            // annoying when wrong language is active
    ...
    // strip word separators from end (may be necessary for urls, e.g. when the user
    // has typed
    ...
    private String stripWordSeparatorsFromEnd(final String word, final SettingsValues settingsValues) {
        ...
        if (settingsValues.mSpacingAndPunctuations.isWordSeparator(word.codePointBefore(word.length()))) {
            ...
            while (endIndex != 0
                    && settingsValues.mSpacingAndPunctuations.isWordSeparator(word.codePointBefore(endIndex)))
                --endIndex;
    ...
    public void performUpdateSuggestionStripSync(final SettingsValues settingsValues, final int inputStyle) {
        ...
        if (suggestedWords != null) {
            // Prefer clipboard suggestions (if available and setting is enabled) over
            // beginning of sentence predictions.
    ...
    public void restartSuggestionsOnWordTouchedByCursor(final SettingsValues settingsValues) {
        ...
        if (!settingsValues.mSpacingAndPunctuations.mCurrentLanguageHasSpaces
                ...
                || !settingsValues.needsToLookupSuggestions()
                // If we are currently in a batch input, we must not resume suggestions, or the
                // result
                // of the batch input will replace the new composition. This may happen in the
                // corner case
                // that the app moves the cursor on its own accord during a batch input.
                || mInputLogicHandler.isInBatchInput()
                // If the cursor is not touching a word, or if there is a selection, return
                // right away.
        ...
        // If for some strange reason (editor bug or so) we measure the text before the
        // cursor as
        // longer than what the entire text is supposed to be, the safe thing to do is
        // bail out.
        if (range.mHasUrlSpans)
            return;
        // If there are links, we don't resume suggestions. Making
        // edits to a linkified text through batch commands would ruin the URL spans,
        // and unless
        // we take very complicated steps to preserve the whole link, we can't do things
        // right so
    ...
    private void restartSuggestions(final TextRange range) {
        ...
        if (suggestions.size() <= 1) {
            ...
            // be 1
            // if shouldIncludeResumedWordInSuggestions is true, 0 otherwise. In this case,
            // we
            ...
            // them, and make willAutoCorrect false. We make typedWordValid false, because
            // the
            // color of the word in the suggestion strip changes according to this
            // parameter,
     ...
     * <p>
     * This is triggered upon pressing backspace just after a commit with
     * auto-correction.
     *
     * @param inputTransaction The transaction in progress.
     */
    private void revertCommit(final InputTransaction inputTransaction) {
        ...
        // TODO: should we check our saved separator against the actual contents of the
        // text view?
        ...
        if (DebugFlags.DEBUG_ENABLED) {
            ...
            final CharSequence wordBeforeCursor = mConnection.getTextBeforeCursor(deleteLength, 0).subSequence(0,
                    cancelLength);
        ...
        if (committedWord instanceof SpannableString committedWordWithSuggestionSpans) {
            ...
            for (final Object span : spans) {
                // If this is a suggestion span, we check that the word is not the committed
                // word.
        ...
        if (inputTransaction.getSettingsValues().mSpacingAndPunctuations.mCurrentLanguageHasSpaces) {
            ...
            // For languages without spaces, we revert the typed string but the cursor is
            // flush
        ...
        // We have a separator between the word and the cursor: we should show
        // predictions.
        inputTransaction.setRequiresUpdateSuggestions();
    }
     ...
     * This is called from the KeyboardSwitcher (through a trampoline in LatinIME)
     * because it
     ...
     * <p>
     * We can resume suggestions on words whose first code point is a word code
     * point (with some
     ...
     * <p>
     * Some keys input a TLD (specifically, the ".com" key) and this warrants some
     * specific
     * processing. First, if this is a TLD, we ignore PHANTOM spaces -- this is done
     * by type
     * of character in onCodeInput, but since this gets inputted as a whole string
     * we need to
     * do it here specifically. Then, if the last character before the cursor is a
     * period, then
     * we cut the dot at the start of ".com". This is because humans tend to type
     * "www.google."
     * and then press the ".com" key and instinctively don't expect to get
     * "www.google..com".
     *
     ...
     * @param clearSuggestionStrip whether this method should clear the suggestion
     *                             strip.
     */
     ...
     * <p>
     * Compare #resetEntireInputState, which also clears the suggestion strip and
     * resets the
     * input connection caches. This only deals with the composing state.
     *
     * @param alsoResetLastComposedWord whether to also reset the last composed
     *                                  word.
     */
    ...
    /**
     * Make a {@link helium314.keyboard.latin.SuggestedWords} object containing a
     * typed word
    ...
    static SuggestedWords retrieveOlderSuggestions(final SuggestedWordInfo typedWordInfo,
        ...
        final ArrayList<SuggestedWords.SuggestedWordInfo> typedWordAndPreviousSuggestions = SuggestedWords
                .getTypedWordAndPreviousSuggestions(typedWordInfo, oldSuggestedWords);
    ...
    /**
     * @return the current {@link Locale} of the {@link #mDictionaryFacilitator} if
     *         available. Otherwise
     *         {@link Locale#ROOT}.
     */
    @NonNull
    ...
    /**
     * Gets a chunk of text with or the auto-correction indicator underline span as
     * appropriate.
     * <p>
     * This method looks at the old state of the auto-correction indicator to put or
     * not put
     * the underline span as appropriate. It is important to note that this does not
     * correspond
     * exactly to whether this word will be auto-corrected to or not: what's
     * important here is
     * to keep the same indication as before.
     * When we add a new code point to a composing word, we don't know yet if we are
     * going to
     * auto-correct it until the suggestions are computed. But in the mean time, we
     * still need
     * to display the character and to extend the previous underline. To avoid any
     * flickering,
     * the underline should keep the same color it used to have, even if that's not
     * ultimately
     * the correct color for this new word. When the suggestions are finished
     * evaluating, we
     * will call this method again to fix the color of the underline.
     *
     * @param text the text on which to maybe apply the span.
     * @return the same text, with the auto-correction underline span if that's
     *         appropriate.
     */
     ...
     * <p>
     * If possible at all, avoid using this method. It causes all sorts of race
     * conditions with
     * the text view because it goes through a different, asynchronous binder. Also,
     * batch edits
     ...
     *
     * @param keyCode   the key code to send inside the key event.
     * @param metaState the meta state to send inside the key event, e.g.
     ...
     * <p>
     * This checks the options and the text before the cursor are appropriate before
     * inserting
     * an automatic space.
     *
     * @param settingsValues the current values of the settings.
    ...
    private void insertAutomaticSpaceIfOptionsAndTextAllow(final SettingsValues settingsValues) {
        if (settingsValues.shouldInsertSpacesAutomatically()
                ...
                && !mConnection.textBeforeCursorLooksLikeURL() // adding this check to textBeforeCursorMayBeUrlOrSimilar
                                                               // might not be wanted for word continuation (see effect
                                                               // on unit tests)
    ...
    private boolean textBeforeCursorMayBeUrlOrSimilar(final SettingsValues settingsValues, final Boolean forAutoSpace) {
        ...
        // we never want to commit the first part of the url, but we want to insert
        // autospace if text might be a normal word
                (forAutoSpace
                        ? mConnection.nonWordCodePointAndNoSpaceBeforeCursor(settingsValues.mSpacingAndPunctuations) // avoid
                                                                                                                     // detecting
                                                                                                                     // URL
                                                                                                                     // if
                                                                                                                     // it
            ...
            return true;
        // already contains a SometimesWordConnector -> may be URL (not so sure, only do
        ...
        final CharSequence textBeforeCursor = mConnection.getTextBeforeCursor(mWordComposer.getTypedWord().length() + 3,
                0);
     ...
     * <p>
     * This is typically called when we should commit the currently composing word
     * without applying
     * auto-correction to it. Typically, we come here upon pressing a separator when
     * the keyboard
     * is configured to not do auto-correction at all (because of the settings or
     * the properties of
     * the editor). In this case, `separatorString' is set to the separator that was
     * pressed.
     * We also come here in a variety of cases with external user action. For
     * example, when the
     * cursor is moved while there is a composition, or when the keyboard is closed,
     * or when the
     * user presses the Send button for an SMS, we don't auto-correct as that would
     * be unexpected.
     * In this case, `separatorString' is set to NOT_A_SEPARATOR.
     *
     * @param settingsValues  the current values of the settings.
     * @param separatorString the separator that's causing the commit, or
     *                        NOT_A_SEPARATOR if none.
     */
     ...
     * <p>
     * This will commit the best guess of the keyboard regarding what the user meant
     * by typing
     * the currently composing word. The IME computes suggestions and assigns a
     * confidence score
     * to each of them; when it's confident enough in one suggestion, it replaces
     * the typed string
     * by this suggestion at commit time. When it's not confident enough, or when it
     * has no
     * suggestions, or when the settings or environment does not allow for
     * auto-correction, then
     * this method just commits the typed string.
     * Note that if suggestions are currently being computed in the background, this
     * method will
     * block until the computation returns. This is necessary for consistency (it
     * would be very
     * strange if pressing space would commit a different word depending on how fast
     * you press).
     *
    ...
    private void commitCurrentAutoCorrection(final SettingsValues settingsValues,
        ...
        if (handler.hasPendingUpdateSuggestions()) {
            handler.cancelUpdateSuggestionStrip();
            // To know the input style here, we should retrieve the in-flight "update
            // suggestions"
            // message and read its arg1 member here. However, the Handler class does not
            // let
            ...
            // input, we update the suggestions synchronously when the tail batch comes.
            // Likewise
            // for application-specified completions. As for recorrections, we never
            // auto-correct,
        ...
        if (stringToCommit != null) {
            ...
            if (!typedWord.equals(stringToCommit)) {
                ...
                // to the user that auto-correction happened. It has no other effect; in
                // particular
                // note that this won't affect the text inside the text field AT ALL: it only
                // makes
     ...
     * @param commitType      the type of the commit, as one of
     *                        LastComposedWord.COMMIT_TYPE_*
     * @param separatorString the separator that's causing the commit, or
     *                        NOT_A_SEPARATOR if none.
     */
    private void commitChosenWord(final SettingsValues settingsValues, final String chosenWord,
        ...
        // When we are composing word, get n-gram context from the 2nd previous word
        // because the
        // 1st previous word is the word to be committed. Otherwise get n-gram context
        // from the 1st
        // previous word.
        ...
        // TODO: figure out here if this is an auto-correct or if the best word is
        // actually
     ...
     * (unfortunately Android long-press selection may select one of those
     * codepoints, so unwrap may not work well)
     */
    private void wrapSelection(final int start, final int end) {
        ...
        if (mConnection.getCodePointBeforeCursor() == start) {
            ...
            if (afterCursor.length() > 0 && afterCursor.charAt(0) == end) {
                mConnection.setSelection(mConnection.getExpectedSelectionStart() - 1,
                        mConnection.getExpectedSelectionEnd() + 1);
     ...
     * <p>
     * When the editor can't be accessed we can't reset the caches, so we schedule a
     * retry.
     * This method handles the retry, and re-schedules a new retry if we still can't
     * access.
    ...
    public boolean retryResetCachesAndReturnSuccess(final boolean tryResumeSuggestions,
        ...
        if (!mConnection.resetCachesUponCursorMoveAndReturnSuccess(
            ...
            // If remainingTries is 0, we should stop waiting for new tries, however we'll
            // still
            // return true as we need to perform other tasks (for example, loading the
            // keyboard).
        }
        mConnection.tryFixIncorrectCursorPosition();
     ...
     * {@link RichInputConnection#setComposingText(CharSequence, int)}, but it is
     * recommended to
    ...
    /**
     * Equivalent to {@link #setComposingTextInternal(CharSequence, int)} except
     * that this method
     * allows to set {@link BackgroundColorSpan} to the composing text with the
     * given color.
     *
     * <p>
     * TODO: Currently the background color is exclusive with the black underline,
     * which is
     * automatically added by the framework. We need to change the framework if we
     * need to have both
     ...
     * @param newComposingText  the composing text to be set
     * @param newCursorPosition the new cursor position
     * @param backgroundColor   the background color to be set to the composing
     *                          text. Set
     *                          {@link Color#TRANSPARENT} to disable the background
     *                          color.
     * @param coloredTextLength the length of text, in Java chars, which should be
     *                          rendered with
    ...
    private void setComposingTextInternalWithBackgroundColor(final CharSequence newComposingText,
        ...
        if (!mConnection.setComposingTextWithBackgroundColor(newComposingText, newCursorPosition,
                backgroundColor, coloredTextLength)) {
    ...
    /**
     * Gets the expected index of the first char of the composing span within the
     * editor's text.
     ...
     * @return The expected index in Java chars of the first char of the composing
     *         span.
     */
    // TODO: try and see if we can get rid of this method. Ideally the users of this
    // class should
    ...
    private void enterInlineEmojiSearchIfNeeded(int codePoint, SettingsValues settingsValues) {
        ...
        if (isStartOfInlineEmojiSearch(codePoint, mConnection.getCodePointBeforeCursor(),
                mConnection.getCharBeforeBeforeCursor(),
                settingsValues)) {
    ...
    private void setInlineEmojiSearchAction(boolean on) {
        if (on != isInlineEmojiSearchAction()) {
            KeyboardSwitcher.getInstance().loadKeyboard(mLatinIME.getCurrentInputEditorInfo(), Settings.getValues(),
                    ...
                    on ? new KeyboardLayoutSet.InternalAction(KeyCode.INLINE_EMOJI_SEARCH_DONE, "!icon/close_history")
                            : null);
        }
    }
    ...
    // public for testing
    public static boolean isStartOfInlineEmojiSearch(int codePoint, int codePointBeforeCursor,
    ...
    private static boolean isValidInlineEmojiSearchPreviousChar(int charBeforeBeforeCursor,
            SettingsValues settingsValues) {
